...
const SHORTCUT_KEYS = [
  { keys: 'Ctrl+Shift+A', actionKey: 'toggleCompact' },
  { keys: 'Ctrl+Shift+F', actionKey: 'toggleFullscreen' },
  { keys: 'Ctrl+Shift+H', actionKey: 'toggleBust' },
  { keys: 'Escape', actionKey: 'close' },
  { keys: 'Enter', actionKey: 'send' },
  { keys: 'Shift+Enter', actionKey: 'newLine' },
]

const MODE_OPTIONS: { value: WidgetMode; labelKey: string }[] = [
  { value: 'fab', labelKey: 'fab' },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-3xl w-full mb-12">
        {FEATURE_KEYS.map((f) => (
          ...
          >
            ...
            <h3 className="text-text-primary font-semibold text-sm mb-1">{t(`landing.features.${f.key}.title`)}</h3>
            <p className="text-text-muted text-xs leading-relaxed">{t(`landing.features.${f.key}.desc`)}</p>
          </div>
        ))}
      </div>
          ...
          {SHORTCUT_KEYS.map((s, i) => (
            <div
              ...
              className={`flex items-center justify-between px-4 py-2 ${
                i < SHORTCUT_KEYS.length - 1 ? 'border-b border-white/[0.03]' : ''
              }`}
            >
              <kbd className="px-2 py-0.5 rounded bg-slate-dark border border-slate-mid/50 text-text-secondary text-xs font-mono">
                {s.keys}
              </kbd>
              <span className="text-text-muted text-xs">{t(`landing.shortcuts.${s.actionKey}`)}</span>
            </div>
          ))}
        </div>
      </div>

...
export function OptionControl({
  ...
  useEffect(() => {
    if (option.type !== 'select' || !option.choices?.length) return
    const choices = filterChoicesForModel(option.choices, model)
    if (!choices.length) return
    const currentVal = value ?? option.defaultValue
    const isValid = choices.some((c) => c.value === String(currentVal))
    if (!isValid) onChange(option.defaultValue as string | number)
  }, [model, option.type, option.choices, option.defaultValue, value, onChange])
